...
downloadable_file <- eventReactive(input$compute,{
    print("downloadable file preparation")
        samples_iterations_computations_tbl <- aggregate_table()
        
        last_column_index <- ncol(samples_iterations_computations_tbl)
        last_row_index <- nrow(samples_iterations_computations_tbl)
        
        
        wb1 <- openxlsx::createWorkbook()
        addWorksheet(wb = wb1, sheetName = "Sheet 1")
        writeData(wb = wb1 , sheet = 1 , x =samples_iterations_computations_tbl )
        style <- openxlsx::createStyle(bgFill = "tan1")

        conditionalFormatting(wb1, sheet = 1,
                              cols = (last_column_index-2):last_column_index,
                              rows = 2:last_row_index,
                              rule = str_c(">=",input$warning_threshold),
                              style = style
        )
        print(str_glue("warning threshold: {n}", n = str_c(">=",input$warning_threshold)))
        print("end of excel file preparation: done")
        
        return(wb1)
        # openxlsx::saveWorkbook(wb1, file = "aggregation_iterations.xlsx", overwrite = T)
    })
    ...
    observeEvent(input$compute,{
        output$downloadData <- downloadHandler(
            ...
            content = function(file) {
                openxlsx::saveWorkbook(wb = downloadable_file(),
                                       file =  file ,
                                       overwrite = T)
